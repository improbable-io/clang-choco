...
$packageName= 'clang' # arbitrary name for the package, used in messages
$toolsDir   = "$(Split-Path -parent $MyInvocation.MyCommand.Definition)"
$url        = 'http://releases.llvm.org/3.8.0/LLVM-3.8.0-win32.exe' # download url, HTTPS preferred
$url64      = 'http://releases.llvm.org/3.8.0/LLVM-3.8.0-win64.exe' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url
...
$packageArgs = @{
  ...
  softwareName  = 'clang*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
